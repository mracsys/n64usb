...
DeviceError device_test_everdrive(CartDevice* cart, USB_DeviceInfoListNode *device_info)
{
    ...
    if (strcmp(device_info->description, "FT245R USB FIFO") == 0 && device_info->id == 0x04036001)
    {
        ...
        device->vid = (uint16_t)(device_info->id >> 16);
        device->pid = (uint16_t)(device_info->id & 0xFFFF);
        memcpy(device->serial, device_info->serial, sizeof(device_info->serial));
        memcpy(device->description, device_info->description, sizeof(device_info->description));
        ...
        // Open the device
        if (device_usb_open(device, &temphandle) != USB_OK || !temphandle)
        {
            free(device);
